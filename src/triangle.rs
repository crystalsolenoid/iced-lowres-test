...
use iced_wgpu::core::{Rectangle, Size, Transformation};
use iced_wgpu::graphics::mesh::{self, Mesh};
use iced_wgpu::graphics::Antialiasing;
use iced_wgpu::wgpu as iwgpu;
...
impl Layer {
    fn new(device: &iwgpu::Device, solid: &solid::Pipeline, gradient: &gradient::Pipeline) -> Self {
        Self {
            index_buffer: Buffer::new(
                device,
                "iced_wgpu.triangle.index_buffer",
                INITIAL_INDEX_COUNT,
                iwgpu::BufferUsages::INDEX | iwgpu::BufferUsages::COPY_DST,
            ),
    ...
    fn prepare(
        ...
        for mesh in meshes {
            ...
            let uniforms = Uniforms::new(transformation * mesh.transformation());
        ...
        for (index, mesh) in meshes.iter().enumerate() {
            ...
            match mesh {
                Mesh::Solid { .. } => {
                    ...
                    render_pass
                        .set_vertex_buffer(0, self.solid.vertices.slice_from_index(num_solids));
...
mod solid {
    ...
    impl Layer {
        pub fn new(device: &iwgpu::Device, constants_layout: &iwgpu::BindGroupLayout) -> Self {
            let vertices = Buffer::new(
                device,
                "iced_wgpu.triangle.solid.vertex_buffer",
                triangle::INITIAL_VERTEX_COUNT,
                iwgpu::BufferUsages::VERTEX | iwgpu::BufferUsages::COPY_DST,
            );
            ...
            let uniforms = Buffer::new(
                ...
                1,
                iwgpu::BufferUsages::UNIFORM | iwgpu::BufferUsages::COPY_DST,
            );
        ...
        pub fn bind_group(
            device: &iwgpu::Device,
            buffer: &iwgpu::Buffer,
            layout: &iwgpu::BindGroupLayout,
        ) -> iwgpu::BindGroup {
            device.create_bind_group(&iwgpu::BindGroupDescriptor {
                label: Some("iced_wgpu.triangle.solid.bind_group"),
                layout,
                entries: &[iwgpu::BindGroupEntry {
                    binding: 0,
                    resource: iwgpu::BindingResource::Buffer(iwgpu::BufferBinding {
                        buffer,
                        offset: 0,
    ...
    impl Pipeline {
        pub fn new(
            ...
            let pipeline = device.create_render_pipeline(&iwgpu::RenderPipelineDescriptor {
                label: Some("iced_wgpu::triangle::solid pipeline"),
                layout: Some(&layout),
                vertex: iwgpu::VertexState {
                    module: &shader,
                    entry_point: "solid_vs_main",
                    buffers: &[iwgpu::VertexBufferLayout {
                        array_stride: std::mem::size_of::<mesh::SolidVertex2D>() as u64,
                        step_mode: iwgpu::VertexStepMode::Vertex,
                        attributes: &iwgpu::vertex_attr_array!(
                            // Position
                            0 => Float32x2,
                            // Color
                            1 => Float32x4,
                ...
                fragment: Some(iwgpu::FragmentState {
                    module: &shader,
